  ...
  return (
    <>
      ...
      <div className="pt-16 sm:pt-24 md:pt-32 lg:pt-[200px] xl:pt-[296px] pb-12 sm:pb-16 md:pb-24 lg:pb-[150px] xl:pb-[200px] text-center border-b border-[#2d2a24] px-4 sm:px-6 md:px-8 lg:px-[40px]">
        ...
        <p className="mt-5 text-base sm:text-lg md:text-xl lg:text-[20px] font-sans font-bold text-[#2d2a24] tracking-[0.04em]" style={{ lineHeight: '2' }}>会社のこと</p>
      </div>
      ...
      <section className="w-full">
        <div className="grid grid-cols-1 md:grid-cols-9">
          <div className='border-b md:border-b-0 md:border-r border-[#2d2a24] col-span-1 md:col-span-4 p-6 sm:p-8 md:p-12 lg:p-16 xl:p-[120px]'>
            <h2 className="font-sans font-bold text-[#2d2a24] tracking-[0.04em] text-xl sm:text-2xl md:text-3xl lg:text-[32px] text-center" style={{ lineHeight: '2' }}>はじめに物語あり</h2>
          </div>
          <div className="col-span-1 md:col-span-5 p-6 sm:p-8 md:p-12 lg:p-16 xl:p-[120px] font-sans text-base sm:text-lg md:text-xl lg:text-[20px] tracking-[0.04em] font-medium text-[#2d2a24]" style={{ lineHeight: '2' }}>
            <p>どこに行けば良いとか、</p>
            <p>何をすれば良いとか、</p>
            <p>どんな風になれば良いとか、</p>
            <p>そんな地図がなくなった時代。</p>
          ...
          {[
            ...
            <div key={item.label} className={`col-span-1 ${index < 2 ? 'border-b md:border-b-0 md:border-r border-[#2d2a24]' : ''}`}>
              ...
              <div className='border-y border-[#2d2a24] pt-[17px] pb-[19px] px-4 sm:px-6 md:px-8 lg:px-[40px]'>
                <p className="font-sans font-medium text-[#2d2a24] tracking-[0.04em] text-sm sm:text-base lg:text-[16px] text-left" style={{ lineHeight: '2' }}>{item.sub}</p>
              </div>
              <div className='py-8 sm:py-12 md:py-16 lg:py-[80px] px-4 sm:px-6 md:px-8 lg:px-[40px]'>
                ...
                {Array.isArray(item.text) ? (
                  item.text.map((textItem, textIndex) => (
                    <li key={textIndex} className="font-sans font-medium text-[#2d2a24] tracking-[0.04em] text-sm sm:text-base lg:text-[16px] text-left" style={{ lineHeight: '2' }}>{textItem}</li>
                  ))
                ) : (
        ...
        <div className='border-b border-[#2d2a24] grid grid-cols-1 lg:grid-cols-9'>
          ...
          <div className="col-span-1 lg:col-span-5">
            ...
            <div className='py-8 md:py-12 lg:py-[80px] px-4 md:px-6 lg:px-[40px] text-[#2d2a24]'>
              {company?.presidentMessage ? company.presidentMessage.split('\n').map((line, index) => (
                <p key={index} className="text-sm md:text-base lg:text-[16px] font-medium tracking-[0.04em] flex justify-start font-sans" style={{ lineHeight: 2 }}>
                  {line}
                </p>
              )) : null}
      ...
      <section className="w-full border-b border-[#2d2a24]">
        <div className='border-b border-[#2d2a24] p-4 sm:p-[17px] pb-4 sm:pb-[19px] px-4 sm:px-6 md:px-8 lg:px-[40px]'>
          <p className="font-sans font-medium text-sm sm:text-base lg:text-[16px] tracking-[0.04em] text-[#333]" style={{ lineHeight: '2' }}>メンバー紹介</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {members.map((m, index) => {
            ...
            return (
            <div key={m.name} className={`flex flex-col items-center text-left p-4 sm:p-6 md:p-8 lg:p-[40px] border-r border-[#2d2a24] ${isLastRowMobile ? '' : 'border-b'} ${isLastRowTablet ? 'sm:border-b-0' : 'sm:border-b'} ${isLastRowDesktop ? 'lg:border-b-0' : 'lg:border-b'}`}>
              <div className="w-full">
                <img src={m.photo?.url} alt={m.name} className="w-full h-auto"/>
              </div>
              <div className="mt-4 sm:mt-6 md:mt-[27px]">
                <p className="font-sans font-bold text-base sm:text-lg md:text-xl lg:text-[20px] tracking-[0.04em] text-[#2d2a24]" style={{ lineHeight: '2' }}>{m.name}</p>
                <p className="font-sans font-bold text-xs sm:text-sm md:text-[14px] tracking-[0.04em] text-[#2d2a24] mt-3" style={{ lineHeight: '2' }}>{m.position}</p>
                <p className="mt-3 sm:mt-3 md:mt-[25px] font-sans font-medium text-sm sm:text-base lg:text-[16px] tracking-[0.04em] text-[#2d2a24]" style={{ lineHeight: '2' }}>{m.profile}</p>
              </div>
            </div>
            )
          })}
      ...
      <section className="w-full">
        <div className="px-4 sm:px-6 md:px-8 lg:px-[40px] border-b border-[#2d2a24] flex flex-col sm:flex-row items-center justify-between min-h-[96px] py-4 sm:py-0 sm:h-[96px]">
          ...
          <div className='flex items-center justify-end h-full w-full sm:w-auto pt-4 sm:pt-0'>
            ...
            <p className='text-sm sm:text-base lg:text-[16px] font-medium text-[#333] tracking-[0.04em] font-sans pl-0 sm:pl-6 md:pl-8 lg:pl-[40px]' style={{ lineHeight: '2' }}>会社概要</p>
          </div>
        </div>
